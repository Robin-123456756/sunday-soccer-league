-- Sunday Soccer League - Reset & Full Setup
-- Run this in the Supabase SQL Editor to replace Prisma-created tables
-- with the full schema including RLS, triggers, indexes, and seed data.

-- Step 1: Drop existing Prisma-created tables (in dependency order)
drop table if exists public.export_jobs cascade;
drop table if exists public.team_sheet_uploads cascade;
drop table if exists public.referee_reports cascade;
...
drop table if exists public.caution_reasons cascade;
drop table if exists public.users cascade;
drop table if exists public.roles cascade;
drop table if exists public.users_profile cascade;

-- Drop Prisma-created enums
drop type if exists public."MatchStatus" cascade;
drop type if exists public."LineupType" cascade;
drop type if exists public."CardType" cascade;
drop type if exists public."ExportType" cascade;
drop type if exists public."ExportFormat" cascade;
drop type if exists public."ExportStatus" cascade;

-- Drop new enums if re-running
drop type if exists public.match_status cascade;
drop type if exists public.lineup_type cascade;
...
-- Step 2: Run full schema setup
-- (Everything below is from schema.sql)
...
-- Team managers can manage uploads for their own team
