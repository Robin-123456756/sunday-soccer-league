...
import { getMatchCardEvents, getMatchDetails, getMatchReport, getMatchUploads } from '@/server/queries/matches';
    ...
    <main style={pageStyle}>
      <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gap: 20 }}>
        ...
        <div style={gridStyle}>
          ...
          <InfoCard title="Score" value={`${match.home_score ?? 0} - ${match.away_score ?? 0}`} helper="Update result handling later if needed" />
          <InfoCard title="Jerseys" value={`${match.home_jersey_color ?? '\u2014'} / ${match.away_jersey_color ?? '\u2014'}`} helper="Home / Away colors" />
          <InfoCard title="Records" value={`${cards.length} cards · ${uploads.length} uploads`} helper={report ? 'Referee report submitted' : 'No referee report yet'} />
        </div>
